                ...
                <div className="fixed inset-0 z-[200] flex items-center justify-end p-0 md:p-4 bg-black/90 backdrop-blur-xl">
                    ...
                    >
                        {profile?.status !== 'approved' && profile?.role !== 'admin' ? (
                            /* ── HOLD SCREEN FOR PENDING USERS ── */
                            <div className="flex-1 p-8 md:p-16 flex flex-col items-center justify-center text-center space-y-10 bg-[#07090D] relative overflow-hidden">
                                <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-primary/50 via-primary to-primary/50"></div>
                                <div className="p-8 bg-primary/10 rounded-[40px] border border-primary/20 shadow-glow relative z-10">
                                    <ShieldCheck className="text-primary" size={64} />
                                </div>
                                <div className="space-y-6 relative z-10">
                                    <h2 className="text-3xl font-black italic-luxury uppercase italic gold-text tracking-tighter">Acceso Restringido</h2>
                                    <p className="text-white/40 text-xs leading-relaxed max-w-sm mx-auto font-light">
                                        Tu expediente de admisión está bajo auditoría.
                                        Solo los miembros aprobados por el Cómite pueden acceder a la logística y servicios de la isla.
                                    </p>
                                </div>
                                <div className="flex flex-col gap-4 w-full px-10 relative z-10">
                                    <div className="py-4 bg-white/5 rounded-2xl border border-white/10 text-[9px] text-white/40 font-black uppercase tracking-[0.3em]">
                                        Estatus: <span className="text-primary">{profile?.status?.toUpperCase() || 'PENDIENTE'}</span>
                                    </div>
                                    <button onClick={onClose} className="btn-primary w-full py-5 text-[10px] tracking-[0.4em] font-black uppercase shadow-glow">ENTENDIDO</button>
                                    <button onClick={onLogout} className="text-[9px] text-red-500/40 hover:text-red-500 transition-colors py-2 uppercase font-black tracking-widest">Cerrar Sesión</button>
                                </div>
                                <div className="pt-20 opacity-10 relative z-10">
                                    <p className="text-[8px] text-white uppercase tracking-[0.8em]">SECURE ADMISSION SYSTEM</p>
                                </div>
                            </div>
                        ) : (
                            <>
                                {/* ── FULL VIP PANEL CONTENT ── */}
